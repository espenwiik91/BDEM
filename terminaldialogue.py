...
def dialogue(sc):
    ...
    print("In this program, tweets from the irma hurricane are analyzed. \n"
          "The dataset contains tweets in the time period from", first, "to", last, "and it contains: ",
          len(readtweets),
          "number of tweets.")
...
def whole_set(sc, readtweets):
    ...
    if choice == 1:
        ...
        make_table(filtered1, "frequency", "word")
    ...
    if choice == 2:
        ...
        make_table(filtered2, "frequency", "word")
    ...
    if choice == 3:
        ...
        if len(rtword) != 0:
            res = (top5_tweets_with_filterword("irmaHurricaneTweets.csv", rtword))
            for r in res:
                print(r)
            while True:
                ...
                if len(choice) != 0:
                    res = (top5_tweets_with_filterword("irmaHurricaneTweets.csv", choice))
                    for r in res:
                        print(r)
                else:
                    break
    ...
    while True:
        try:
            repeat = int(input(
                "Do you want to stop analyzing(1), do another analysis on this dataset(2), "
                "or do an anlysis on a subset of the dataset (3)\n"))
...
def divide_dataset(sc, readtweets):
    ...
    while True:
        try:
            ...
            stop = int(input("stop\n"))
            assert 0 <= start < stop and stop <= len(readtweets)
            break
        except:
    ...
    if choice == 2:
        ...
        print("Here are the 10 most disaster-related words combination of two words in this dataset"
              ". A bar chart is shown in SciView")
        make_table(filtered2, "frequency:", "bigrams:")
        make_histogram(filtered2)
        print("\n")
    if choice == 3:
        rtword = str(input(
            ("This function lets you find the most retweeted tweets - overall or containing a chosen keyword\n"
             ...
             "Type a word, or hit enter. \n")))
        if len(rtword) != 0:
            res = (top5_tweets_with_filterword("temp.csv", rtword))
            for r in res:
                print(r)
            while True:
                print("Type a new word if you want to try another keyword, or hit enter to proceed")
                choice = str(input("Type a word and/or hit enter\n"))
                if len(choice) != 0:
                    res = (top5_tweets_with_filterword("temp.csv", choice))
                    for r in res:
                        print(r)
                else:
                    break
    ...
    if choice == 5:
        ...
        print("Here are the 10 most disaster-related words in this dataset"
              ". A bar chart is shown in SciView")
        make_table(filtered3, "frequency:", "word:")
        make_histogram(filtered3)
        print("Here are the 10 most disaster-related words combination of two words in this dataset"
              ". A bar chart is shown in SciView")
        make_table(filtered4, "frequency:", "bigrams:")
        make_histogram(filtered4)
        print("\n")
        print("This function might take some time. Don't press anything until URLs are shown\n"
              "Top five URLs:")
...
"""
    ***************************************************************************************************************
    Takes a file, a start point and a endpoint as arguments. Takes contet from the file between line number at start
    point and line number at the endpoint, and writes this content to a csv file named "temp.csv"
"""
...
"""
    ***********************************************************************************************
    Takes a csv file as argument, and returns a list with the five most frequent ReTweets in the csv file.
"""
...
def top5_tweets(csvFileWithTweetTextAndTweetDate):
    df = pd.read_csv(csvFileWithTweetTextAndTweetDate,
                     ...
                     encoding='utf-8')
    filtered_tweetText = []
    # Goes through every line of the tweet text values and appends it to a list and counts the most common.
...
"""
    ********************************************************************************************************************
    Versatile function that can take in any list of tuples and visualize using a bar chart. Functions on both single 
    words and bigrams.
"""
...
"""
    **************************************************************************************************
    Takes a list of tuples (word frequency(int), word(string)) and uses the list to create a scatterplot
    The visualization is not working properly -- > not called in the dialogue.
"""
